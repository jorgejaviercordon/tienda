<html>
  <head>
    <title>Lista completa de Provedores</title>
  </head>
  <body>
    <h1>Proveedores de la tienda:<br></h1>
...
try {
    ...
    foreach ($conn->query($SQLSelect) as $row) {
      $ProvedorNuevo = new proveedor($row['Nombre'],$row['Cif'],$row['Direccion'],$row['Email'],$row['Telefono']);
      echo $ProvedorNuevo->imprimeProveedor();
    }
